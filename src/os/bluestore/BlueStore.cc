...
int BlueStore::_open_db(bool create, bool to_repair_db)
{
  ...
  if (do_bluefs) {//用bluefs
    ...
    if (bluefs_shared_bdev == BlueFS::BDEV_SLOW) {
      ...
      kv_options["db_paths"] = db_paths.str();//保存文件名
  ...
  WriteContext *wctx)
{
  ...
  if (offset / min_alloc_size == (end - 1) / min_alloc_size &&
    ...
    head_offset = offset;
    //取对齐后，头部多出来的长度(从offset到head_length是左沿，需要“小写"）
    head_length = p2nphase(offset, min_alloc_size);

    tail_offset = p2align(end, min_alloc_size);//取规范的尾部长度
    //取对齐后，尾部多出来的长度(从tail_offset到tail_length是右沿，需要“小写”）
