...
int BlueFS::_replay(bool noop, bool to_stdout)//重演日志
{
...
int BlueFS::_flush_range(FileWriter *h, uint64_t offset, uint64_t length)
{
  ...
  //这里lenght可以为0，需要考虑跳过。
  ceph_assert(offset <= h->file->fnode.size);//offset一定小于size
  ...
  *_dout << dendl;
  ...
  while (length > 0) {
    uint64_t x_len = std::min(p->length - x_off, length);//length是要写入的长度，p->length是这一块的长度，取两者小者
