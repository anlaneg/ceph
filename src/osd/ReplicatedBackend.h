...
public:
  ...
  public:
    bool operator()(const set<pg_shard_t> &have) const override {//只要集合不为空,就可以恢复
      return !have.empty();
    }
  };
  IsPGRecoverablePredicate *get_is_recoverable_predicate() override {//幅本可恢复性检查.
  ...
  public:
    ...
    bool operator()(const set<pg_shard_t> &have) const override {//如果have集合中有主,就可以读
      return have.count(whoami);
    }
  };
  IsPGReadablePredicate *get_is_readable_predicate() override {//幅本可读性检测回调
