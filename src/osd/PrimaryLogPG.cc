...
bool PrimaryLogPG::is_missing_object(const hobject_t& soid) const//missing表中存在这个对象,表明这个对象缺失
{
  return pg_log.get_missing().get_items().count(soid);
}

//读写流程触发,产生优先恢复某object的需求
void PrimaryLogPG::maybe_kick_recovery(//尝试优先恢复soid
  const hobject_t &soid)
{
  eversion_t v;
...
void PrimaryLogPG::wait_for_unreadable_object(//触发soid恢复,并将此操作加入等待可读object列表
...
void PrimaryLogPG::wait_for_degraded_object(const hobject_t& soid, OpRequestRef op)//主动触发要求恢复,并将操作加入到等待降级
{
...
void PrimaryLogPG::block_write_on_degraded_snap(//将此对象加入到等待队列,原因是snap对应的head不存在.
...
bool PrimaryLogPG::maybe_await_blocked_snapset(//如果此对象的head或者snapdir处于阻塞中,则返回true,否则返回false
  const hobject_t &hoid,
  OpRequestRef op)
{
  ObjectContextRef obc;
...
bool PrimaryLogPG::pg_op_must_wait(MOSDOp *op)//如果有当前pg有missing集且PGLS等操作非head对象
{
...
void PrimaryLogPG::do_pg_op(OpRequestRef op)//处理pg的操作
{
...
int PrimaryLogPG::do_scrub_ls(MOSDOp *m, OSDOp *osd_op)//显示snap错误信息
{
...
void PrimaryLogPG::do_cache_redirect(OpRequestRef op)//cache操作重定向
{
  MOSDOp *m = static_cast<MOSDOp*>(op->get_req());
  int flags = m->get_flags() & (CEPH_OSD_FLAG_ACK|CEPH_OSD_FLAG_ONDISK);
...
void PrimaryLogPG::cancel_proxy_ops(bool requeue)//当主备发生切换，当osd需要关闭时，需要cancel
{
...
void PrimaryLogPG::finish_proxy_write(hobject_t oid, ceph_tid_t tid, int r)//向客户端发送写成功消息。
{
  dout(10) << __func__ << " " << oid << " tid " << tid
	   << " " << cpp_strerror(r) << dendl;
...
void PrimaryLogPG::make_writeable(OpContext *ctx)//本函数进行时,快照上下文比对象快照版本小的情况已排除
{
...
void PrimaryLogPG::process_copy_chunk(hobject_t oid, ceph_tid_t tid, int r)//处理copy
{
  dout(10) << __func__ << " " << oid << " tid " << tid
	   << " " << cpp_strerror(r) << dendl;
...
void PrimaryLogPG::finish_promote(int r, CopyResults *results,//提升完成后执行
				  ObjectContextRef obc)
{
  const hobject_t& soid = obc->obs.oi.soid;
...
ObjectContextRef PrimaryLogPG::get_object_context(const hobject_t& soid,//获取soid对应的hobject，如果不存在，则尝试创建（仅head)
						  bool can_create,
						  map<string, bufferlist> *attrs)
{
  assert(
...
SnapSetContext *PrimaryLogPG::get_snapset_context(//如果ssc存在,返回ssc,如果ssc不存在,如果需要创建,则创建ssc,否则返回NULL
  const hobject_t& oid,
  bool can_create,
  map<string, bufferlist> *attrs,
  bool oid_existed)
...
bool PrimaryLogPG::agent_choose_mode(bool restart, OpRequestRef op)//依据pg的配置信息，决定agent的刷新及驱赶模式。
{
  bool requeued = false;
  // Let delay play out
