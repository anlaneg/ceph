  ...
  RPGHandle *h)
{
  ...
  map<hobject_t, set<pg_shard_t>>::const_iterator q = missing_loc.find(soid);//找谁上有它
                                     ...
                                     bool cache_dont_need)
{
  ...
  if (progress.first) {
    int r = store->omap_get_header(ch, ghobject_t(recovery_info.soid), &out_op->omap_header);//读omap_head
    if(r < 0) {
