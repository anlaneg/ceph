  ...
  for (int o = 0; o < osdmap.get_max_osd(); o++) {
    ...
    if (osdmap.is_down(o)) {//o处于down状态，但还没有out
      ...
      if (found == down_pending_out.end()) {//为了发现osd被out掉，我们在检测到osd　down掉后，记录在此数组
        ...
        down_pending_out[o] = ceph_clock_now();//保存down的时间
      }
    } else {
...
void OSDMonitor::share_map_with_random_osd()
{
  ...
  MOSDMap *m = build_incremental(osdmap.get_epoch() - 1, osdmap.get_epoch(), features);//间隔为１
...
MOSDMap *OSDMonitor::build_incremental(epoch_t from, epoch_t to, uint64_t features)
{
  ...
  for (epoch_t e = to; e >= from && e > 0; e--) {
    bufferlist bl;
    int err = get_version(e, features, bl);//获取e版本对应的数据（增量）
    if (err == 0) {
      ...
      ceph_assert(!bl.length());
      get_version_full(e, features, bl);//获取e版本对应的数据（全量）
      if (bl.length() > 0) {
