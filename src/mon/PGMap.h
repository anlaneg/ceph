...
public:
  ...
  mempool::pgmap::unordered_map<int32_t,osd_stat_t> osd_stat;//每一个osd的状态
  mempool::pgmap::unordered_map<pg_t,pg_stat_t> pg_stat;//每一个pg的状态
  mempool::pgmap::set<int32_t> full_osds;     // for pre-luminous only//bigger than full_ratio的所有osd
  mempool::pgmap::set<int32_t> nearfull_osds; // for pre-luminous only//bigger than nearfull_osds的所有osd
  float full_ratio;//full时的ratio
  float nearfull_ratio;//近满的ratio

  // mapping of osd to most recently reported osdmap epoch
  mempool::pgmap::unordered_map<int32_t,epoch_t> osd_epochs;//记录某个osd最近上报时对应的epoch
  ...
  mempool::pgmap::unordered_map<int,set<pg_t> > pg_by_osd;//可针对osd查此osd对应(负责的）的所有pg列表
  mempool::pgmap::unordered_map<int,int> blocked_by_sum;//记录被某一具体osd阻塞的pg数量
 ...
 public:
  ...
  mempool::pgmap::set<pg_t> creating_pgs;//记录正处于creating阶段的pg
  mempool::pgmap::map<int,map<epoch_t,set<pg_t> > > creating_pgs_by_osd_epoch;//osd（注）,epoch到正在创建的Ｐg的映射
