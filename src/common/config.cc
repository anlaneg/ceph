				    ...
				    int flags)
{
  ...
  std::list<std::string> conf_files;//配置文件地址
  ...
  _get_my_sections(values, my_sections);//加载name,name.type,global三个段
  for (const auto &i : schema) {
    const auto &opt = i.second;
    std::string val;
    int ret = _get_val_from_conf_file(my_sections, opt.name, val);//不展开元数据情况下获取value
    if (ret == 0) {
      std::string error_message;
      int r = _set_val(values, tracker, val, opt, CONF_FILE, &error_message);//设置值.
...
{
  if (out) {
    *out << "name = " << values.name << std::endl;//显示进程类型及id
    *out << "cluster = " << values.cluster << std::endl;//显示cluster
  }
  if (f) {
